...
def login(cid,cpw):
    conn=Oracles.oraconn()
    cs = Oracles.oracs(conn)
    sql='select * from coffeemem'
    cs.execute(sql)
    members=cs.fetchall()
    check=0
    for member in members:
        if member[0] == cid and str(member[1]) ==cpw:
            check =1
        else:
            continue
    return check
...
def payplus(cid):#이용권 충전
    print()
    print('[  이용권 충전  ]')
    conn=Oracles.oraconn()
    cs = Oracles.oracs(conn)
    sql="select money from coffeemem where id='{}'".format(cid)
    cs.execute(sql)
    moneycurr=cs.fetchall()
    print(moneycurr)
    print('잔여 금액 : {}'.format(moneycurr[0][0]))
    plpay=int(input('충전할 금액을 입력하세요(원) : '))
    newmoney =moneycurr[0][0]+plpay
    msql = 'update coffeemem set money =:1 where id=:2'
    cs.execute(msql,(newmoney,cid))
    cs.close()
    conn.close()
    print('충전 후 금액 : {}'.format(newmoney))
